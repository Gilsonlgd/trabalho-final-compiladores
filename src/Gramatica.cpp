...
bool Gramatica::addTerminal(char terminal) {
    ...
    if (islower(terminal) || (isdigit(terminal))) {
...
bool Gramatica::addProducao(char naoTerminal, const std::vector<std::string>& producao) {
...
void Gramatica::setErro(const std::string& erro) const {
    this->erro = erro;
}

void Gramatica::printErro() const {
